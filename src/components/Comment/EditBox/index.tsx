...
const EditBox: React.FC<Props> = ({
  ...
  const submit = async () => {
    try {
      ...
      const config = {
        ...
        postTitle: searchParams.get("title") || "",
        replyId: replyId || "",
      };
  ...
  const { run: informAdminMsg } = useRequest(
      ...
      }),
    {
      ...
      onSuccess: () => {
        setText("");
        message.success(
          `发布${searchParams.get("title") ? "评论" : "留言"}成功！`
        );
      },
    }
  );
  ...
  const { run: informUser } = useRequest(
      ...
      }),
    {
      ...
      onSuccess: () => {
        message.success(
          `已通知${searchParams.get("title") ? "评论" : "留言"}者！`
        );
      },
    }
  );
  ...
  return (
    <div className={classNames(styles.editBox, className)}>
      {isReply && (
      ...
      <div className={styles.flex}>
        ...
        <div className={styles.avatarBoxCol}>
          <div className={styles.avatarBox}>
            {avatar ? (
              <img src={avatar} className={styles.editAvatar} />
            ) : (
              <UserOutlined className={styles.noAvatar} />
            )}
          </div>
        </div>
        <div className={styles.editInputBox}>
          <div className={styles.inputBox}>
            <div className={classNames(styles.inputInfo, styles.flex2)}>
            ...
            <div className={classNames(styles.inputInfo, styles.flex3)}>
          ...
          <div className={styles.commentBtns}>
            <Emoji />
            {isReply && (
              <div className={styles.cancelBtn} onClick={handleCloseReply}>
                取消
              </div>
            )}
            <div className={styles.previewBtn} onClick={openPreShow}>
              预览
            </div>
            <div className={styles.sendBtn} onClick={submit}>
              {isReply ? "回复" : " 发布"}
            </div>
          </div>
        </div>
      ...
      <PreShow
        closePre={closePre}
        content={text}
        className={classNames({ [styles.preShowHidden]: !showPre })}
      />
    </div>
  );
};
